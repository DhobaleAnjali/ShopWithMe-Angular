...
export class ProductService {
  ...
  getProductsForSearchUsingAi(): Observable<any> {
    return this.http.get<any>(this.baseUrl).pipe(
      catchError(() => {
        // fallback to local JSON
        return this.getProductsForAiSearch();
      }),
    );
  }

  getProductsForAiSearch(): Observable<any> {
    return this.http.get('/assets/data/allProductsAiSearch.json');
  }
  ...
  getAllGeneralProducts(): Observable<any[]> {
    const home$ = this.http.get<any>(`${this.baseUrl}/category/home-decoration`).pipe(
      catchError((err) => {
    ...
    const furniture$ = this.http.get<any>(`${this.baseUrl}/category/furniture`).pipe(
      catchError((err) => {
    ...
    const electronics$ = this.http.get<any>(`${this.baseUrl}/category/smartphones`).pipe(
      catchError((err) => {
    ...
    const skincare$ = this.http.get<any>(`${this.baseUrl}/category/skincare`).pipe(
      catchError((err) => {
